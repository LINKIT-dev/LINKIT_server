...
public record LinkVO(UserVO userVO, String title, String content, String linkPreviewImg) {
    // TODO: mock 데이터 추후 삭제
    ...
    public static List<LinkVO> ofList(List<Link> links) {
        return links.stream()
                .map(link ->
                            new LinkVO(new UserVO(link.getUser().getUid(), link.getUser().getProfileImg()),
                                    link.getTitle(),
                                    link.getContent(),
                                    link.getPreviewImg()))
